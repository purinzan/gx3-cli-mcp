...
# trace元入力版の前後照合（2026-09-06）

比較元413c6b5、変更版60cba65。同一macOS/Python 3.14.4、固定6検体×3 fresh
processを順次測定。入力hash/options一致、全phaseで既存時間/heap/RSS予算内。
traceで入力hash読込みが2→4（multi-pou8→16、ld-st4→8）。解読済みの
rows/comments/labelsは各1回のまま。SQL/openは変更しない。中央値ms:

| 検体 | cold | warm10 | trace | dependency-flow |
|---|---:|---:|---:|---:|
| small | 60.83→58.96 | 50.87→49.74 | 11.39→13.57 | 3.98→3.82 |
| wide | 91.13→88.10 | 61.83→61.88 | 67.21→69.59 | 29.38→28.55 |
| deep | 88.32→87.55 | 51.61→52.20 | 94.54→96.57 | 31.60→31.74 |
| large-span | 134.78→129.89 | 52.38→49.15 | 11.44→13.34 | 19.29→18.34 |
| multi-pou | 95.78→90.63 | 54.56→52.64 | 92.70→95.99 | 31.76→31.06 |
| ld-st | 92.84→90.09 | 53.09→51.35 | 93.51→95.29 | 31.85→30.74 |

途中版792076fは追加照合のheap予算を超えた。小ファイルにも1MBを要求していた
file_digestの読取りを、ファイルサイズに応じた64KiB〜1MiBへ変更した後の結果が上表。
trace peakはsmall1,207,057→277,274 bytes、wide1,269,268→669,159、
deep1,269,524→630,544、large-span1,208,696→270,895、
multi-pou1,265,820→628,591、ld-st1,270,778→632,342。
GCの変更や予算緩和は行わない。追加照合は実行時間の費用を持つため高速化とは呼ばない。
OS cache未flush、RSSは累積process peak。巨大入力・実設備・Windowsの実測保証ではない。
# 通信CSV依存引継ぎの作業中評価（未合格、2026-09-06）

比較元8ad961a、候補3810c86。固定6検体×3、同じmacOS/Python3.14.4。
候補の機能テスト全80件は成功したが、性能の受入は未完了である。
canonical全phaseの時間/RSSは予算内。warm_queryのPython peakは約140KB増え、
全6検体で既定heap予算を約9〜11KB超過した。以前のGC感度と似るが、原因を断定せず
不合格として保持する。予算やGC設定を変更していない。

別測定: 同じ6種LDDBに64refresh範囲+1units行のCSVを明示して実lite build/prepare、
別CWDでtrace APIからJSONを生成。各3 fresh process、中央値ms:

| 検体 | build/prepare | trace/JSON |
|---|---:|---:|
| small | 99.12→83.10 | 15.51→18.99 |
| wide | 127.29→120.05 | 74.28→92.71 |
| deep | 122.14→112.34 | 99.31→120.77 |
| large-span | 164.57→152.80 | 14.14→18.95 |
| multi-pou | 138.57→119.68 | 104.95→124.47 |
| ld-st | 126.42→116.68 | 102.63→122.07 |

wideのtraceは既定時間上限91.13msを超過。旧版は指定CSVを発見できず、新版は
正しく読み込むため実施する仕事も異なるが、追加費用を隠して合格とはしない。
SQLはtrace+1、CSV内容hashは+6（CSV2本を初回/入口/返却前に追加照合）。
