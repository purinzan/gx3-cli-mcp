...
| `test_gx3_xref_reader_boundary.py` | 新しい読み手が生の `where device=?` を書いたら落ちること、member 索引が範囲の全デバイスを持つこと、範囲途中を問えば命令が見つかり範囲外では見つからないこと、member 表が無いDBでも落ちずに縮退すること、長さ不明の範囲は先頭1件のみになることを検査する。 |
