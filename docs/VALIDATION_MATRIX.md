...
異なるラベル表に同名ラベルがある場合、解析では `名前@LabelID` で区別します。
`trace-device` に曖昧な名前だけを渡した場合は未評価とし、scopeを付けた候補を案内します。
同名ラベルがない場合は従来の名前を使用できます。この区別は合成ケースでのCLI回帰確認であり、GX Works3との独立照合完了を意味しません。
変更前に作成したxrefは再構築してください。
