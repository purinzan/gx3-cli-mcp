...
from gx3cli.gx3_ladder_layout import build_layout_payload, layouts_to_svg
...
def build(
    ...
    payload = build_layout_payload(root, program, None, device)
    ...
    if total > len(kept):
        state = AnalysisState(
            ...
            next_step=f"raise --limit, or narrow it with --device",
        )
    elif partial:
        state = AnalysisState(
            PARTIAL,
...
function select(name) {
  ...
  if (d.project.write > writes.length || d.project.read > reads.length) {
    html += '<p class="counts">この画面はプログラム 1 本分です。全体の一覧は ' +
            '<code>gx3-cli xref where-used ' + name + '</code></p>';
  }
...
def render_html(report: Report) -> str:
    ...
    for rung in report.rungs:
        ...
        rung_html.append(
            f'<div class="rung" data-pos="{esc(rung["pos"])}" data-devices="{esc(devices)}">'
